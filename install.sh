...
initDNSAPIConfig() {
    if [[ "$1" == "cloudflare" ]]; then
        echoContent yellow "\n CF_Token参考配置教程：https/1701160377972\n"
...
manageCDN() {
    ...
    if echo "${currentInstallProtocolType}" | grep -qE ",1,|,2,|,3,|,5,|,11,"; then
        ...
        echoContent yellow "# 注意事项"
        echoContent yellow "\n教程地址:"
        echoContent skyBlue "https/cloudflarezi-xuan-ip"
...
ipv6Routing() {
    ...
    if [[ "${ipv6Status}" == "1" ]]; then
        ...
        echoContent yellow "# 注意事项\n"
        echoContent yellow "# 注意事项"
        echoContent yellow "#  ：https/1683226921000 \n"
...
socks5Routing() {
    ...
    echoContent yellow "#  ：https/1683226921000#heading-5 \n"
...
initRealityDest() {
    if [[ -n "${domain}" ]]; then
        ...
        echoContent green "回落域名列表：https/1680104902581#heading-8\n"
...
initRealityClientServersName() {
    ...
    if [[ -z "${realityServerName}" ]]; then
        ...
        if [[ -z "${realityServerName}" ]]; then
			...
			echoContent yellow "推荐:itunes.apple.com\n"
            read -r -p "请输入目标域名，[回车]随机伪装，默认端口443:" realityServerName
            if [[ -z "${realityServerName}" ]]; then
                randomNum=$(randomNum 1 27)
...
manageReality() {
    ...
    if ! echo "${currentInstallProtocolType}" | grep -q -E "7,|8," || [[ -z "${coreInstallType}" ]]; then
        echoContent red "\n ---> 请先安装Reality协议，  https/1680104902581#heading-11"
        exit 0
    fi
